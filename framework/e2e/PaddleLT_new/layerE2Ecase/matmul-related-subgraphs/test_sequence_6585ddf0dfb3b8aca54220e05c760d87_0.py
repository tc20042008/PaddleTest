...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4):
        ...
        # pd_op.slice: (1820x3x49x32xf32) <- (3x1820x3x49x32xf32, 1xi64, 1xi64)
        t8 = paddle._C_ops.slice(t0, [0], t2, t6, [1], [0])
        
        # pd_op.full_int_array: (1xi64) <- ()
        t9 = [3]
        
        # pd_op.slice: (1820x3x49x32xf32) <- (3x1820x3x49x32xf32, 1xi64, 1xi64)
        t10 = paddle._C_ops.slice(t0, [0], t6, t9, [1], [0])
        
        # pd_op.full: (1xf32) <- ()
        t11 = paddle._C_ops.full([1], float('0.176777'), paddle.float32, paddle.core.CPUPlace())
        
        # pd_op.scale: (1820x3x49x32xf32) <- (1820x3x49x32xf32, 1xf32)
        t12 = paddle._C_ops.scale(t5, t11, float('0'), True)
        del t5
        
        # pd_op.transpose: (1820x3x32x49xf32) <- (1820x3x49x32xf32)
        t13 = paddle._C_ops.transpose(t8, [0, 1, 3, 2])
        del t8
        
        # pd_op.matmul: (1820x3x49x49xf32) <- (1820x3x49x32xf32, 1820x3x32x49xf32)
        t14 = paddle._C_ops.matmul(t12, t13, False, False)
        
        # pd_op.flatten: (2401xi64) <- (49x49xi64)
        t15 = paddle._C_ops.flatten(t3, 0, 1)
        del t3
        
        # pd_op.index_select: (2401x3xf32) <- (169x3xf32, 2401xi64)
        t16 = paddle._C_ops.index_select(t4, t15, 0)
        del t4
        ...
        t19 = paddle._C_ops.transpose(t18, [2, 0, 1])
        del t18
        
        # pd_op.unsqueeze: (1x3x49x49xf32) <- (3x49x49xf32, 1xi64)
        t20 = paddle._C_ops.unsqueeze(t19, t1)
        
        # pd_op.add: (1820x3x49x49xf32) <- (1820x3x49x49xf32, 1x3x49x49xf32)
        t21 = paddle._C_ops.add(t14, t20)
        
        return t6, t7, t9, t10, t11, t12, t13, t14, t15, t16, t19, t20, t21

    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[3, 1820, 3, 49, 32], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([3, 1820, 3, 49, 32], dtype='float32', min=0, max=0.5),
            # t1
