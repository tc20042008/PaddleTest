...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, t18, t19, t20, t21, t22, t23):
        t6 = paddle._C_ops.full_int_array([0], paddle.int64, paddle.core.CPUPlace())
        t7 = paddle._C_ops.full_int_array([1], paddle.int64, paddle.core.CPUPlace())
        t8 = paddle._C_ops.full_int_array([2], paddle.int64, paddle.core.CPUPlace())
        t9 = paddle._C_ops.full_int_array([3], paddle.int64, paddle.core.CPUPlace())
        t24 = 0.17677700519561768
        # builtin.combine: ([xi64, xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64, xi64)
        t25 = [t0, t1, t2, t3, t4]
        del t3
        
        # pd_op.stack: (5xi64) <- ([xi64, xi64, xi64, xi64, xi64])
        t26 = paddle._C_ops.stack(t25, 0)
        del t25
        
        # pd_op.reshape: (-1x-1x3x-1x32xf32) <- (-1x-1x1152xf32, 5xi64)
        t27 = paddle._C_ops.reshape(t5, t26)
        del t5, t26
        
        # pd_op.transpose: (3x-1x-1x-1x32xf32) <- (-1x-1x3x-1x32xf32)
        t28 = paddle._C_ops.transpose(t27, [2, 0, 3, 1, 4])
        del t27
        
        # pd_op.slice: (-1x-1x-1x32xf32) <- (3x-1x-1x-1x32xf32, 1xi64, 1xi64)
        t29 = paddle._C_ops.slice(t28, [0], t6, t7, [1], [0])
        
        # pd_op.slice: (-1x-1x-1x32xf32) <- (3x-1x-1x-1x32xf32, 1xi64, 1xi64)
        t30 = paddle._C_ops.slice(t28, [0], t7, t8, [1], [0])
        
        # pd_op.slice: (-1x-1x-1x32xf32) <- (3x-1x-1x-1x32xf32, 1xi64, 1xi64)
        t31 = paddle._C_ops.slice(t28, [0], t8, t9, [1], [0])
        del t28
        
        # pd_op.transpose: (-1x-1x32x-1xf32) <- (-1x-1x-1x32xf32)
        t32 = paddle._C_ops.transpose(t30, [0, 1, 3, 2])
        del t30
        
        # pd_op.matmul: (-1x-1x-1x-1xf32) <- (-1x-1x-1x32xf32, -1x-1x32x-1xf32)
        t33 = paddle._C_ops.matmul(t29, t32, False, False)
        del t29, t32
        
        # pd_op.scale: (-1x-1x-1x-1xf32) <- (-1x-1x-1x-1xf32, 1xf32)
        t34 = paddle._C_ops.scale(t33, t24, float('0'), True)
        del t33
        
        # pd_op.softmax: (-1x-1x-1x-1xf32) <- (-1x-1x-1x-1xf32)
        t35 = paddle._C_ops.softmax(t34, -1)
        del t34
        
        # pd_op.matmul: (-1x-1x-1x32xf32) <- (-1x-1x-1x-1xf32, -1x-1x-1x32xf32)
        t36 = paddle._C_ops.matmul(t35, t31, False, False)
        del t31, t35
        
        # pd_op.transpose: (-1x-1x-1x32xf32) <- (-1x-1x-1x32xf32)
        t37 = paddle._C_ops.transpose(t36, [0, 2, 1, 3])
        del t36
        ...
        t40 = paddle._C_ops.reshape(t37, t39)
        del t39, t37
        
        # pd_op.matmul: (-1x-1x384xf32) <- (-1x-1x-1xf32, 384x384xf32)
        t41 = paddle._C_ops.matmul(t40, t11, False, False)
        del t11, t40
        
        # pd_op.add: (-1x-1x384xf32) <- (-1x-1x384xf32, 384xf32)
        t42 = paddle._C_ops.add(t41, t12)
        del t41, t12
        
        # pd_op.add: (-1x-1x384xf32) <- (-1x-1x384xf32, -1x-1x384xf32)
        t43 = paddle._C_ops.add(t13, t42)
        del t42, t13
        
        # pd_op.layer_norm: (-1x-1x384xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x384xf32, 384xf32, 384xf32)
        t44, t45, t46 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t43, t14, t15, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t43, t15, t14
        
        # pd_op.matmul: (-1x-1x1536xf32) <- (-1x-1x384xf32, 384x1536xf32)
        t47 = paddle._C_ops.matmul(t44, t16, False, False)
        del t16
        
        # pd_op.add: (-1x-1x1536xf32) <- (-1x-1x1536xf32, 1536xf32)
        t48 = paddle._C_ops.add(t47, t17)
        del t47, t17
        
        # pd_op.gelu: (-1x-1x1536xf32) <- (-1x-1x1536xf32)
        t49 = paddle._C_ops.gelu(t48, False)
        del t48
        
        # pd_op.matmul: (-1x-1x384xf32) <- (-1x-1x1536xf32, 1536x384xf32)
        t50 = paddle._C_ops.matmul(t49, t18, False, False)
        del t49, t18
        
        # pd_op.add: (-1x-1x384xf32) <- (-1x-1x384xf32, 384xf32)
        t51 = paddle._C_ops.add(t50, t19)
        del t50, t19
        
        # pd_op.add: (-1x-1x384xf32) <- (-1x-1x384xf32, -1x-1x384xf32)
        t52 = paddle._C_ops.add(t44, t51)
        del t51, t44
        
        # pd_op.layer_norm: (-1x-1x384xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x384xf32, 384xf32, 384xf32)
        t53, t54, t55 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t52, t20, t21, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t52, t21, t20
        
        # pd_op.matmul: (-1x-1x1152xf32) <- (-1x-1x384xf32, 384x1152xf32)
        t56 = paddle._C_ops.matmul(t53, t22, False, False)
        del t22
        
        # pd_op.add: (-1x-1x1152xf32) <- (-1x-1x1152xf32, 1152xf32)
        t57 = paddle._C_ops.add(t56, t23)
        del t56, t23
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            ...
            # t5
            paddle.uniform([8, 240, 1152], dtype='float32', min=0, max=0.5),
            # t6
