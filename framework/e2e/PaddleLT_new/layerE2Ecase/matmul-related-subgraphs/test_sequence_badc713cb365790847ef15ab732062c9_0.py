...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12):
        ...
        t13 = paddle._C_ops.slice(t0, [0], t1, t2, [1], [0])
        del t0
        
        # pd_op.conv2d: (-1x1024x16x16xf32) <- (-1x3x224x224xf32, 1024x3x14x14xf32)
        t14 = paddle._C_ops.conv2d(t3, t4, [14, 14], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')
        del t3, t4
        
        # pd_op.shape64: (4xi64) <- (-1x1024x16x16xf32)
        t15 = paddle._C_ops.shape64(t14)
        
        # pd_op.slice: (xi64) <- (4xi64, 1xi64, 1xi64)
        t16 = paddle._C_ops.slice(t15, [0], t1, t2, [1], [0])
        del t15
        
        # pd_op.flatten: (-1x1024x256xf32) <- (-1x1024x16x16xf32)
        t17 = paddle._C_ops.flatten(t14, 2, 3)
        ...
        # pd_op.full: (xi64) <- ()
        t19 = paddle._C_ops.full([], float('-1'), paddle.int64, paddle.core.CPUPlace())
        ...
        t22 = paddle._C_ops.expand(t5, t21)
        del t5
        
        # pd_op.full: (1xi32) <- ()
        t23 = paddle._C_ops.full([1], float('1'), paddle.int32, paddle.core.CPUPlace())
        ...
        t25 = paddle._C_ops.concat(t24, t23)
        del t24
        ...
        t27, t28, t29 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t26, t7, t8, float('1e-05'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t8, t7
        
        # pd_op.layer_norm: (-1x257x1024xf32, -1x257xf32, -1x257xf32) <- (-1x257x1024xf32, 1024xf32, 1024xf32)
        t30, t31, t32 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t27, t9, t10, float('1e-05'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t10, t9
        
        # pd_op.shape64: (3xi64) <- (-1x257x1024xf32)
        t33 = paddle._C_ops.shape64(t30)
        
        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        t34 = paddle._C_ops.slice(t33, [0], t1, t2, [1], [0])
        del t33
        
        # pd_op.matmul: (-1x257x3072xf32) <- (-1x257x1024xf32, 1024x3072xf32)
        t35 = paddle._C_ops.matmul(t30, t11, False, False)
        del t11
        
        # pd_op.add: (-1x257x3072xf32) <- (-1x257x3072xf32, 3072xf32)
        t36 = paddle._C_ops.add(t35, t12)
        del t12
        ...
        t39 = paddle._C_ops.transpose(t38, [2, 0, 3, 1, 4])
        del t38
        
        # pd_op.slice: (-1x16x257x64xf32) <- (3x-1x16x257x64xf32, 1xi64, 1xi64)
        t40 = paddle._C_ops.slice(t39, [0], t1, t2, [1], [0])
        
        # pd_op.full_int_array: (1xi64) <- ()
        t41 = [2]
        
        return t14, t18, t21, t22, t23, t25, t26, t27, t28, t29, t30, t31, t32, t35, t36, t37, t39, t40, t41
    ...
    def get_input_spec(self):
        return [
            ...
            # t4
            paddle.static.InputSpec(shape=[1024, 3, 14, 14], dtype='float32'),
            # t5
            paddle.static.InputSpec(shape=[1, 1, 1024], dtype='float32'),
            # t6
            paddle.static.InputSpec(shape=[1, 257, 1024], dtype='float32'),
            # t7
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.to_tensor([8, 3, 224, 224], dtype='int64').reshape([4]),
            # t1
            paddle.to_tensor([0], dtype='int64').reshape([1]),
            # t2
            paddle.to_tensor([1], dtype='int64').reshape([1]),
            # t3
            paddle.uniform([8, 3, 224, 224], dtype='float32', min=0, max=0.5),
            # t4
            paddle.uniform([1024, 3, 14, 14], dtype='float32', min=0, max=0.5),
            # t5
            paddle.uniform([1, 1, 1024], dtype='float32', min=0, max=0.5),
            # t6
            paddle.uniform([1, 257, 1024], dtype='float32', min=0, max=0.5),
            # t7
