...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2):
        ...
        t4 = paddle._C_ops.slice(t0, [0], t1, t3, [1], [0])
        
        # pd_op.full_int_array: (1xi64) <- ()
        t5 = [3]
        
        # pd_op.slice: (-1x12x-1x32xf32) <- (3x-1x12x-1x32xf32, 1xi64, 1xi64)
        t6 = paddle._C_ops.slice(t0, [0], t3, t5, [1], [0])
        del t0
        
        # pd_op.transpose: (-1x12x32x-1xf32) <- (-1x12x-1x32xf32)
        t7 = paddle._C_ops.transpose(t4, [0, 1, 3, 2])
        del t4
        
        # pd_op.matmul: (-1x12x-1x-1xf32) <- (-1x12x-1x32xf32, -1x12x32x-1xf32)
        t8 = paddle._C_ops.matmul(t2, t7, False, False)
        del t2, t7
    ...
    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[3, None, 12, None, 32], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([3, 8, 12, 240, 32], dtype='float32', min=0, max=0.5),
            # t1
