...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14):
        ...
        t15 = paddle._C_ops.unsqueeze(t0, t1)
        del t0
        
        # pd_op.multiply: (64x96x56x56xf32) <- (96x1x1xf32, 64x96x56x56xf32)
        t16 = paddle._C_ops.multiply(t15, t2)
        
        # pd_op.unsqueeze: (96x1x1xf32) <- (96xf32, 2xi64)
        t17 = paddle._C_ops.unsqueeze(t3, t1)
        del t3
        
        # pd_op.add: (64x96x56x56xf32) <- (64x96x56x56xf32, 96x1x1xf32)
        t18 = paddle._C_ops.add(t16, t17)
        
        # pd_op.depthwise_conv2d: (64x96x56x56xf32) <- (64x96x56x56xf32, 96x1x7x7xf32)
        t19 = paddle._C_ops.depthwise_conv2d(t18, t4, [1, 1], [3, 3], 'EXPLICIT', 96, [1, 1], 'NCHW')
        del t4
        
        # pd_op.reshape: (1x96x1x1xf32) <- (96xf32, 4xi64)
        t20 = paddle._C_ops.reshape(t5, t6)
        del t5
        
        # pd_op.add: (64x96x56x56xf32) <- (64x96x56x56xf32, 1x96x1x1xf32)
        t21 = paddle._C_ops.add(t19, t20)
        
        # pd_op.transpose: (64x56x56x96xf32) <- (64x96x56x56xf32)
        t22 = paddle._C_ops.transpose(t21, [0, 2, 3, 1])
        del t21
        
        # pd_op.layer_norm: (64x56x56x96xf32, 64x56x56xf32, 64x56x56xf32) <- (64x56x56x96xf32, 96xf32, 96xf32)
        t23, t24, t25 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t22, t7, t8, float('1e-06'), 3), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t8, t7
        
        # pd_op.matmul: (64x56x56x384xf32) <- (64x56x56x96xf32, 96x384xf32)
        t26 = paddle._C_ops.matmul(t23, t9, False, False)
        del t9
        
        # pd_op.add: (64x56x56x384xf32) <- (64x56x56x384xf32, 384xf32)
        t27 = paddle._C_ops.add(t26, t10)
        del t10
        
        # pd_op.gelu: (64x56x56x384xf32) <- (64x56x56x384xf32)
        t28 = paddle._C_ops.gelu(t27, False)
        
        # pd_op.matmul: (64x56x56x96xf32) <- (64x56x56x384xf32, 384x96xf32)
        t29 = paddle._C_ops.matmul(t28, t11, False, False)
        del t11
        
        # pd_op.add: (64x56x56x96xf32) <- (64x56x56x96xf32, 96xf32)
        t30 = paddle._C_ops.add(t29, t12)
        del t12
        
        # pd_op.multiply: (64x56x56x96xf32) <- (96xf32, 64x56x56x96xf32)
        t31 = paddle._C_ops.multiply(t13, t30)
        del t13
        
        # pd_op.transpose: (64x96x56x56xf32) <- (64x56x56x96xf32)
        t32 = paddle._C_ops.transpose(t31, [0, 3, 1, 2])
        del t31
        
        # pd_op.add: (64x96x56x56xf32) <- (64x96x56x56xf32, 64x96x56x56xf32)
        t33 = paddle._C_ops.add(t18, t32)
        
        # pd_op.depthwise_conv2d: (64x96x56x56xf32) <- (64x96x56x56xf32, 96x1x7x7xf32)
        t34 = paddle._C_ops.depthwise_conv2d(t33, t14, [1, 1], [3, 3], 'EXPLICIT', 96, [1, 1], 'NCHW')
        del t14
        
        return t15, t16, t17, t18, t19, t20, t22, t23, t24, t25, t26, t27, t28, t29, t30, t32, t33, t34
    ...
    def get_input_spec(self):
        return [
            ...
            # t2
            paddle.static.InputSpec(shape=[64, 96, 56, 56], dtype='float32'),
            # t3
            paddle.static.InputSpec(shape=[96], dtype='float32'),
            # t4
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            ...
            # t2
            paddle.uniform([64, 96, 56, 56], dtype='float32', min=0, max=0.5),
            # t3
            paddle.uniform([96], dtype='float32', min=0, max=0.5),
            # t4
