...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, t18, t19, t20, t21):
        ...
        t35 = paddle._C_ops.transpose(t33, [0, 1, 3, 2])
        del t33
        
        # pd_op.matmul: (-1x8x-1x-1xf32) <- (-1x8x-1x32xf32, -1x8x32x-1xf32)
        t36 = paddle._C_ops.matmul(t32, t35, False, False)
        del t32, t35
        
        # pd_op.scale: (-1x8x-1x-1xf32) <- (-1x8x-1x-1xf32, 1xf32)
        t37 = paddle._C_ops.scale(t36, t22, float('0'), True)
        del t36
        
        # pd_op.softmax: (-1x8x-1x-1xf32) <- (-1x8x-1x-1xf32)
        t38 = paddle._C_ops.softmax(t37, -1)
        del t37
        
        # pd_op.matmul: (-1x8x-1x32xf32) <- (-1x8x-1x-1xf32, -1x8x-1x32xf32)
        t39 = paddle._C_ops.matmul(t38, t34, False, False)
        del t38, t34
        
        # pd_op.transpose: (-1x-1x8x32xf32) <- (-1x8x-1x32xf32)
        t40 = paddle._C_ops.transpose(t39, [0, 2, 1, 3])
        del t39
        ...
        t43 = paddle._C_ops.reshape(t40, t42)
        del t42, t40
        
        # pd_op.matmul: (-1x-1x256xf32) <- (-1x-1x256xf32, 256x256xf32)
        t44 = paddle._C_ops.matmul(t43, t11, False, True)
        del t11, t43
        ...
        t46 = paddle._C_ops.add(t13, t45)
        del t13, t45
        
        # pd_op.layer_norm: (-1x-1x256xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x256xf32, 256xf32, 256xf32)
        t47, t48, t49 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t46, t14, t15, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t15, t14
        
        # pd_op.matmul: (-1x-1x1024xf32) <- (-1x-1x256xf32, 256x1024xf32)
        t50 = paddle._C_ops.matmul(t47, t16, False, False)
        del t47, t16
        
        # pd_op.add: (-1x-1x1024xf32) <- (-1x-1x1024xf32, 1024xf32)
        t51 = paddle._C_ops.add(t50, t17)
        del t50, t17
        
        # pd_op.gelu: (-1x-1x1024xf32) <- (-1x-1x1024xf32)
        t52 = paddle._C_ops.gelu(t51, False)
        del t51
        
        # pd_op.matmul: (-1x-1x256xf32) <- (-1x-1x1024xf32, 1024x256xf32)
        t53 = paddle._C_ops.matmul(t52, t18, False, False)
        del t52, t18
        
        # pd_op.add: (-1x-1x256xf32) <- (-1x-1x256xf32, 256xf32)
        t54 = paddle._C_ops.add(t53, t19)
        del t53, t19
        
        # pd_op.add: (-1x-1x256xf32) <- (-1x-1x256xf32, -1x-1x256xf32)
        t55 = paddle._C_ops.add(t46, t54)
        del t46, t54
        
        # pd_op.layer_norm: (-1x-1x256xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x256xf32, 256xf32, 256xf32)
        t56, t57, t58 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t55, t20, t21, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t21, t20
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.to_tensor([10, 29, 256], dtype='int64').reshape([3]),
            # t1
            ...
            # t4
            paddle.uniform([10, 29, 256], dtype='float32', min=0, max=0.5),
            # t5
