...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3):
        ...
        t5 = paddle._C_ops.reshape(t0, t4)
        del t0, t4
        ...
        t15 = paddle._C_ops.strided_slice(t5, [1, 2], t6, t7, t11)
        
        # pd_op.full_int_array: (2xi64) <- ()
        t16 = [1, 0]
        
        # pd_op.strided_slice: (2x120x88x96xf32) <- (2x240x176x96xf32, 2xi64, 2xi64, 2xi64)
        t17 = paddle._C_ops.strided_slice(t5, [1, 2], t16, t7, t11)
        
        # pd_op.full_int_array: (2xi64) <- ()
        t18 = [0, 1]
        
        # pd_op.strided_slice: (2x120x88x96xf32) <- (2x240x176x96xf32, 2xi64, 2xi64, 2xi64)
        t19 = paddle._C_ops.strided_slice(t5, [1, 2], t18, t7, t11)
        
        # pd_op.full_int_array: (2xi64) <- ()
        t20 = [1, 1]
        
        # pd_op.strided_slice: (2x120x88x96xf32) <- (2x240x176x96xf32, 2xi64, 2xi64, 2xi64)
        t21 = paddle._C_ops.strided_slice(t5, [1, 2], t20, t7, t11)
        
        # pd_op.full: (1xi32) <- ()
        t22 = paddle._C_ops.full([1], float('-1'), paddle.int32, paddle.core.CPUPlace())
        ...
        t24 = paddle._C_ops.concat(t23, t22)
        del t23
        ...
        t26 = paddle._C_ops.reshape(t24, t25)
        del t25
        
        # pd_op.layer_norm: (2x10560x384xf32, 2x10560xf32, 2x10560xf32) <- (2x10560x384xf32, 384xf32, 384xf32)
        t27, t28, t29 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t26, t1, t2, float('1e-05'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t2, t1
        
        # pd_op.matmul: (2x10560x192xf32) <- (2x10560x384xf32, 384x192xf32)
        t30 = paddle._C_ops.matmul(t27, t3, False, False)
        del t3
        
        return t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, t18, t19, t20, t21, t22, t24, t26, t27, t28, t29, t30

    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[2, 42240, 96], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([2, 42240, 96], dtype='float32', min=0, max=0.5),
            # t1
