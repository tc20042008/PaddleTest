...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4):
        ...
        t14 = paddle._C_ops.slice(t9, [0], t10, t12, [1], [0])
        ...
        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        t17 = paddle._C_ops.slice(t9, [0], t12, t15, [1], [0])
        
        # pd_op.full_int_array: (1xi64) <- ()
        t18 = [3]
        
        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        t19 = paddle._C_ops.slice(t9, [0], t15, t18, [1], [0])
        
        # pd_op.full: (1xf32) <- ()
        t20 = paddle._C_ops.full([1], float('0.176777'), paddle.float32, paddle.core.CPUPlace())
        
        # pd_op.scale: (126x12x49x32xf32) <- (126x12x49x32xf32, 1xf32)
        t21 = paddle._C_ops.scale(t14, t20, float('0'), True)
        del t14
        
        # pd_op.transpose: (126x12x32x49xf32) <- (126x12x49x32xf32)
        t22 = paddle._C_ops.transpose(t17, [0, 1, 3, 2])
        del t17
        
        # pd_op.matmul: (126x12x49x49xf32) <- (126x12x49x32xf32, 126x12x32x49xf32)
        t23 = paddle._C_ops.matmul(t21, t22, False, False)
        
        # pd_op.flatten: (2401xi64) <- (49x49xi64)
        t24 = paddle._C_ops.flatten(t3, 0, 1)
        del t3
        
        # pd_op.index_select: (2401x12xf32) <- (169x12xf32, 2401xi64)
        t25 = paddle._C_ops.index_select(t4, t24, 0)
        del t4
        ...
        t28 = paddle._C_ops.transpose(t27, [2, 0, 1])
        del t27
        
        # pd_op.unsqueeze: (1x12x49x49xf32) <- (12x49x49xf32, 1xi64)
        t29 = paddle._C_ops.unsqueeze(t28, t10)
        
        # pd_op.add: (126x12x49x49xf32) <- (126x12x49x49xf32, 1x12x49x49xf32)
        t30 = paddle._C_ops.add(t23, t29)
        
        return t5, t6, t9, t10, t11, t12, t13, t15, t16, t18, t19, t20, t21, t22, t23, t24, t25, t28, t29, t30

    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[126, 49, 384], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([126, 49, 384], dtype='float32', min=0, max=0.5),
            # t1
