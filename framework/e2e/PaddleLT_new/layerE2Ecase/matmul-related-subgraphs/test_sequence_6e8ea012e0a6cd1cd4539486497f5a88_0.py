...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8):
        ...
        t9 = paddle._C_ops.conv2d(t0, t1, [1, 1], [2, 2], 'EXPLICIT', [1, 1], 4, 'NCHW')
        del t1
        ...
        t12 = paddle._C_ops.add(t9, t11)
        del t9, t11
        
        # pd_op.add: (-1x128x-1x80xf32) <- (-1x-1x-1x80xf32, -1x128x-1x80xf32)
        t13 = paddle._C_ops.add(t0, t12)
        del t12, t0
        
        # pd_op.flatten: (-1x128x-1xf32) <- (-1x128x-1x80xf32)
        t14 = paddle._C_ops.flatten(t13, 2, 3)
        del t13
        
        # pd_op.transpose: (-1x-1x128xf32) <- (-1x128x-1xf32)
        t15 = paddle._C_ops.transpose(t14, [0, 2, 1])
        del t14
        
        # pd_op.layer_norm: (-1x-1x128xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x128xf32, 128xf32, 128xf32)
        t16, t17, t18 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t15, t3, t4, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t4, t3, t15
        
        # pd_op.matmul: (-1x-1x512xf32) <- (-1x-1x128xf32, 128x512xf32)
        t19 = paddle._C_ops.matmul(t16, t5, False, False)
        del t5
        
        # pd_op.add: (-1x-1x512xf32) <- (-1x-1x512xf32, 512xf32)
        t20 = paddle._C_ops.add(t19, t6)
        del t19, t6
        
        # pd_op.gelu: (-1x-1x512xf32) <- (-1x-1x512xf32)
        t21 = paddle._C_ops.gelu(t20, False)
        del t20
        
        # pd_op.matmul: (-1x-1x128xf32) <- (-1x-1x512xf32, 512x128xf32)
        t22 = paddle._C_ops.matmul(t21, t7, False, False)
        del t21, t7
