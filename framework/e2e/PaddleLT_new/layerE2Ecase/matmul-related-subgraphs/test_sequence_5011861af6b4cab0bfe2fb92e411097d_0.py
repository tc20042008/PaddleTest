...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4):
        ...
        # pd_op.full: (1xf32) <- ()
        t8 = paddle._C_ops.full([1], float('0.125'), paddle.float32, paddle.core.CPUPlace())
    ...
    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[3, None, 16, 257, 64], dtype='float32'),
            # t1
            paddle.static.InputSpec(shape=[1], dtype='int64'),
            # t2
            paddle.static.InputSpec(shape=[1], dtype='int64'),
            # t3
            paddle.static.InputSpec(shape=[None, 16, 257, 64], dtype='float32'),
            # t4
            paddle.static.InputSpec(shape=[None, 16, 257, 64], dtype='float32'),
        ]
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([3, 128, 16, 257, 64], dtype='float32', min=0, max=0.5),
            # t1
            paddle.to_tensor([2], dtype='int64').reshape([1]),
            # t2
            paddle.to_tensor([3], dtype='int64').reshape([1]),
            # t3
            paddle.uniform([128, 16, 257, 64], dtype='float32', min=0, max=0.5),
            # t4
            paddle.uniform([128, 16, 257, 64], dtype='float32', min=0, max=0.5),
        ]
