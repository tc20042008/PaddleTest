...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4):
        ...
        t5 = paddle._C_ops.mean(t0, [-2, -1], False)
        
        # pd_op.layer_norm: (32x768xf32, 32xf32, 32xf32) <- (32x768xf32, 768xf32, 768xf32)
        t6, t7, t8 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t5, t1, t2, float('1e-06'), 1), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t2, t1
        
        # pd_op.matmul: (32x102xf32) <- (32x768xf32, 768x102xf32)
        t9 = paddle._C_ops.matmul(t6, t3, False, False)
        del t3
