...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14):
        ...
        t13 = paddle._C_ops.full_int_array([-1, 7, 7, 768], paddle.int64, paddle.core.CPUPlace())
        t14 = paddle._C_ops.full_int_array([-1, 1, 1, 7, 7, 768], paddle.int64, paddle.core.CPUPlace())
        ...
        t23 = paddle._C_ops.reshape(t20, t22)
        del t22
        
        # pd_op.softmax: (-1x24x49x49xf32) <- (-1x24x49x49xf32)
        t24 = paddle._C_ops.softmax(t23, -1)
        del t23
        
        # pd_op.matmul: (-1x24x49x32xf32) <- (-1x24x49x49xf32, -1x24x49x32xf32)
        t25 = paddle._C_ops.matmul(t24, t9, False, False)
        
        # pd_op.transpose: (-1x49x24x32xf32) <- (-1x24x49x32xf32)
        t26 = paddle._C_ops.transpose(t25, [0, 2, 1, 3])
        del t25
        ...
        t30 = paddle._C_ops.matmul(t29, t11, False, False)
        del t11
        
        # pd_op.add: (-1x49x768xf32) <- (-1x49x768xf32, 768xf32)
        t31 = paddle._C_ops.add(t30, t12)
        del t12
        
        # pd_op.reshape: (-1x7x7x768xf32) <- (-1x49x768xf32, 4xi64)
        t32 = paddle._C_ops.reshape(t31, t13)
        
        # pd_op.reshape: (-1x1x1x7x7x768xf32) <- (-1x7x7x768xf32, 6xi64)
        t33 = paddle._C_ops.reshape(t32, t14)
        del t14
        
        # pd_op.transpose: (-1x1x7x1x7x768xf32) <- (-1x1x1x7x7x768xf32)
        t34 = paddle._C_ops.transpose(t33, [0, 1, 3, 2, 4, 5])
        del t33
        
        # pd_op.reshape: (-1x7x7x768xf32) <- (-1x1x7x1x7x768xf32, 4xi64)
        t35 = paddle._C_ops.reshape(t34, t13)
        del t13
        
        return t17, t19, t20, t24, t26, t29, t30, t31, t32, t34, t35
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.to_tensor(60, dtype='int64').reshape([]),
            # t1
            ...
            # t4
            paddle.uniform([60, 24, 49, 49], dtype='float32', min=0, max=0.5),
            # t5
            ...
            paddle.to_tensor(60, dtype='int64').reshape([]),
            # t9
            paddle.uniform([60, 24, 49, 32], dtype='float32', min=0, max=0.5),
            # t10
