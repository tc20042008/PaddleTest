...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4):
        # pd_op.matmul: (2x2352x1536xf32) <- (2x2352x384xf32, 384x1536xf32)
        t5 = paddle._C_ops.matmul(t0, t1, False, False)
        del t0, t1
        
        # pd_op.add: (2x2352x1536xf32) <- (2x2352x1536xf32, 1536xf32)
        t6 = paddle._C_ops.add(t5, t2)
        del t2
        
        # pd_op.gelu: (2x2352x1536xf32) <- (2x2352x1536xf32)
        t7 = paddle._C_ops.gelu(t6, False)
        
        # pd_op.matmul: (2x2352x384xf32) <- (2x2352x1536xf32, 1536x384xf32)
        t8 = paddle._C_ops.matmul(t7, t3, False, False)
        del t3
        
        # pd_op.add: (2x2352x384xf32) <- (2x2352x384xf32, 384xf32)
    ...
    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[2, 2352, 384], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([2, 2352, 384], dtype='float32', min=0, max=0.5),
            # t1
