...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4):
        # pd_op.matmul: (2x-1x3072xf32) <- (2x-1x-1xf32, 768x3072xf32)
        t5 = paddle._C_ops.matmul(t0, t1, False, False)
        del t0, t1
        
        # pd_op.add: (2x-1x3072xf32) <- (2x-1x3072xf32, 3072xf32)
        t6 = paddle._C_ops.add(t5, t2)
        del t2
        
        # pd_op.gelu: (2x-1x3072xf32) <- (2x-1x3072xf32)
        t7 = paddle._C_ops.gelu(t6, False)
        
        # pd_op.matmul: (2x-1x768xf32) <- (2x-1x3072xf32, 3072x768xf32)
    ...
    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[2, None, None], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([2, 660, 768], dtype='float32', min=0, max=0.5),
            # t1
