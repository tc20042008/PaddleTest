...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16):
        ...
        t9 = paddle._C_ops.full_int_array([0], paddle.int64, paddle.core.CPUPlace())
        t10 = paddle._C_ops.full_int_array([3], paddle.int64, paddle.core.CPUPlace())
        t17 = 0.17677700519561768
        t18 = None
        t19 = 0.10000000149011612
        # pd_op.shape64: (3xi64) <- (4x-1x384xf32)
        t20 = paddle._C_ops.shape64(t0)
        
        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        t21 = paddle._C_ops.slice(t20, [0], t1, t2, [1], [0])
        del t20
        
        # pd_op.matmul: (4x-1x1152xf32) <- (4x-1x384xf32, 384x1152xf32)
        t22 = paddle._C_ops.matmul(t0, t3, False, False)
        del t3
        
        # pd_op.add: (4x-1x1152xf32) <- (4x-1x1152xf32, 1152xf32)
        t23 = paddle._C_ops.add(t22, t4)
        del t4
        ...
        t28 = paddle._C_ops.slice(t27, [0], t9, t1, [1], [0])
        
        # pd_op.slice: (4x12x-1x32xf32) <- (3x4x12x-1x32xf32, 1xi64, 1xi64)
        t29 = paddle._C_ops.slice(t27, [0], t1, t2, [1], [0])
        
        # pd_op.slice: (4x12x-1x32xf32) <- (3x4x12x-1x32xf32, 1xi64, 1xi64)
        t30 = paddle._C_ops.slice(t27, [0], t2, t10, [1], [0])
        
        # pd_op.transpose: (4x12x32x-1xf32) <- (4x12x-1x32xf32)
        t31 = paddle._C_ops.transpose(t29, [0, 1, 3, 2])
        del t29
        
        # pd_op.matmul: (4x12x-1x-1xf32) <- (4x12x-1x32xf32, 4x12x32x-1xf32)
        t32 = paddle._C_ops.matmul(t28, t31, False, False)
        
        # pd_op.scale: (4x12x-1x-1xf32) <- (4x12x-1x-1xf32, 1xf32)
        t33 = paddle._C_ops.scale(t32, t17, float('0'), True)
        del t32
        
        # pd_op.add: (4x12x-1x-1xf32) <- (4x12x-1x-1xf32, 1x1x-1x-1xf32)
        t34 = paddle._C_ops.add(t33, t11)
        ...
        t36 = paddle._C_ops.matmul(t35, t30, False, False)
        ...
        t41 = paddle._C_ops.matmul(t40, t13, False, False)
        del t13
        
        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        t42 = paddle._C_ops.add(t41, t14)
        del t14
        
        # pd_op.dropout: (4x-1x384xf32, 4x-1x384xui8) <- (4x-1x384xf32, None, 1xf32)
        t43, t44 = (lambda x, f: f(x))(paddle._C_ops.dropout(t42, None, t19, False, 'upscale_in_train', 0, False), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
        del t42
        
        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 4x-1x384xf32)
        t45 = paddle._C_ops.add(t0, t43)
        
        # pd_op.layer_norm: (4x-1x384xf32, 4x-1xf32, 4x-1xf32) <- (4x-1x384xf32, 384xf32, 384xf32)
        t46, t47, t48 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t45, t15, t16, float('1e-05'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t16, t15
        
        # pd_op.shape64: (3xi64) <- (4x-1x384xf32)
        t49 = paddle._C_ops.shape64(t46)
        
        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        t50 = paddle._C_ops.slice(t49, [0], t1, t2, [1], [0])
        del t49
        
        return t22, t23, t27, t28, t30, t31, t33, t35, t37, t40, t41, t43, t44, t45, t46, t47, t48, t50
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            ...
            # t11
            paddle.uniform([1, 1, 13, 13], dtype='float32', min=0, max=0.5),
            # t12
