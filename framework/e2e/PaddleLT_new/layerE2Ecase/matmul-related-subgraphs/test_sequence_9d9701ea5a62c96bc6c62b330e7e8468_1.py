...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4):
        # pd_op.matmul: (2x-1x1024xf32) <- (2x-1x256xf32, 256x1024xf32)
        t5 = paddle._C_ops.matmul(t0, t1, False, False)
        del t0, t1
        
        # pd_op.add: (2x-1x1024xf32) <- (2x-1x1024xf32, 1024xf32)
        t6 = paddle._C_ops.add(t5, t2)
        del t2
        
        # pd_op.relu: (2x-1x1024xf32) <- (2x-1x1024xf32)
        t7 = paddle._C_ops.relu(t6)
        del t6
        
        # pd_op.matmul: (2x-1x256xf32) <- (2x-1x1024xf32, 1024x256xf32)
