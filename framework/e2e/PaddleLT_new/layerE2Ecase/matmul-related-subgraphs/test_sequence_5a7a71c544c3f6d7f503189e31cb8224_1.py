...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6):
        ...
        t7 = paddle._C_ops.slice(t0, [1], t1, t2, [1], [1])
        
        # pd_op.full_int_array: (1xi64) <- ()
        t8 = [2147483647]
        
        # pd_op.slice: (16x256x1024xf32) <- (16x257x1024xf32, 1xi64, 1xi64)
        t9 = paddle._C_ops.slice(t0, [1], t2, t8, [1], [])
        del t8
        
        # pd_op.layer_norm: (16x1024xf32, 16xf32, 16xf32) <- (16x1024xf32, 1024xf32, 1024xf32)
        t10, t11, t12 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t7, t3, t4, float('1e-05'), 1), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t4, t3
        
        # pd_op.matmul: (16x102xf32) <- (16x1024xf32, 1024x102xf32)
        t13 = paddle._C_ops.matmul(t10, t5, False, False)
        del t5
        
        # pd_op.add: (16x102xf32) <- (16x102xf32, 102xf32)
        t14 = paddle._C_ops.add(t13, t6)
        del t6
        
        return t7, t10, t11, t12, t13, t14

    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[16, 257, 1024], dtype='float32'),
            # t1
            ...
            paddle.static.InputSpec(shape=[1024], dtype='float32'),
            # t5
            paddle.static.InputSpec(shape=[1024, 102], dtype='float32'),
            # t6
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([16, 257, 1024], dtype='float32', min=0, max=0.5),
            # t1
            ...
            paddle.uniform([1024], dtype='float32', min=0, max=0.5),
            # t5
            paddle.uniform([1024, 102], dtype='float32', min=0, max=0.5),
            # t6
