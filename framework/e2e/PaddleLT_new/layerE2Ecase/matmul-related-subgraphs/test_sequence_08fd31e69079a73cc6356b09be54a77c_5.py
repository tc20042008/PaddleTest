...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14):
        ...
        # pd_op.shape64: (3xi64) <- (-1x49x512xf32)
        ...
        # pd_op.reshape: (-1x49x3x16x32xf32) <- (-1x49x1536xf32, 5xi64)
        t22 = paddle._C_ops.reshape(t19, t21)
        del t19, t21
        
        # pd_op.transpose: (3x-1x16x49x32xf32) <- (-1x49x3x16x32xf32)
        t23 = paddle._C_ops.transpose(t22, [2, 0, 3, 1, 4])
        del t22
        
        # pd_op.slice: (-1x16x49x32xf32) <- (3x-1x16x49x32xf32, 1xi64, 1xi64)
        ...
        # pd_op.scale: (-1x16x49x32xf32) <- (-1x16x49x32xf32, 1xf32)
        t27 = paddle._C_ops.scale(t24, t15, float('0'), True)
        del t24
        
        # pd_op.transpose: (-1x16x32x49xf32) <- (-1x16x49x32xf32)
        t28 = paddle._C_ops.transpose(t25, [0, 1, 3, 2])
        del t25
        
        # pd_op.matmul: (-1x16x49x49xf32) <- (-1x16x49x32xf32, -1x16x32x49xf32)
        ...
        # pd_op.index_select: (2401x16xf32) <- (169x16xf32, 2401xi64)
        t31 = paddle._C_ops.index_select(t13, t30, 0)
        del t13, t30
        
        # pd_op.reshape: (49x49x16xf32) <- (2401x16xf32, 3xi64)
        t32 = paddle._C_ops.reshape(t31, t14)
        del t31
        
        # pd_op.transpose: (16x49x49xf32) <- (49x49x16xf32)
        t33 = paddle._C_ops.transpose(t32, [2, 0, 1])
        del t32
        
        # pd_op.unsqueeze: (1x16x49x49xf32) <- (16x49x49xf32, 1xi64)
        t34 = paddle._C_ops.unsqueeze(t33, t1)
        del t33
        
        # pd_op.add: (-1x16x49x49xf32) <- (-1x16x49x49xf32, 1x16x49x49xf32)
    ...
    def get_input_spec(self):
        return [
            ...
            paddle.static.InputSpec(shape=[1], dtype='int64'),
            # t13
            paddle.static.InputSpec(shape=[169, 16], dtype='float32'),
            # t14
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([256, 49, 512], dtype='float32', min=0, max=0.5),
            # t1
            ...
            paddle.to_tensor([-1], dtype='int64').reshape([1]),
            # t13
            paddle.uniform([169, 16], dtype='float32', min=0, max=0.5),
            # t14
