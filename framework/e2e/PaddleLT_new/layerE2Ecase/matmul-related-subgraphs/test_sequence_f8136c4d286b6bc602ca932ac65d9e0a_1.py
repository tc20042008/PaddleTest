...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13):
        ...
        t14, t15, t16 = (lambda x, f: f(x))(paddle._C_ops.distribute_fpn_proposals(t0, t1, 2, 5, 4, 224, False), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t0, t1
        ...
        # pd_op.roi_align: (-1x256x7x7xf32) <- (1x256x176x264xf32, -1x4xf32, -1xi32)
        t25 = paddle._C_ops.roi_align(t2, t17, t21, 7, 7, float('0.25'), 0, True)
        del t2
        
        # pd_op.roi_align: (-1x256x7x7xf32) <- (1x256x88x132xf32, -1x4xf32, -1xi32)
        t26 = paddle._C_ops.roi_align(t3, t18, t22, 7, 7, float('0.125'), 0, True)
        del t3
        
        # pd_op.roi_align: (-1x256x7x7xf32) <- (1x256x44x66xf32, -1x4xf32, -1xi32)
        t27 = paddle._C_ops.roi_align(t4, t19, t23, 7, 7, float('0.0625'), 0, True)
        del t4
        
        # pd_op.roi_align: (-1x256x7x7xf32) <- (1x256x22x33xf32, -1x4xf32, -1xi32)
        t28 = paddle._C_ops.roi_align(t5, t20, t24, 7, 7, float('0.03125'), 0, True)
        del t5
        
        # pd_op.full: (1xi32) <- ()
        t29 = paddle._C_ops.full([1], float('0'), paddle.int32, paddle.core.CPUPlace())
        ...
        t32 = paddle._C_ops.concat(t31, t29)
        del t31
        
        # pd_op.gather: (-1x256x7x7xf32) <- (-1x256x7x7xf32, -1x1xi32, 1xi32)
        t33 = paddle._C_ops.gather(t32, t16, t29)
        
        # pd_op.flatten: (-1x12544xf32) <- (-1x256x7x7xf32)
        t34 = paddle._C_ops.flatten(t33, 1, 3)
        
        # pd_op.matmul: (-1x1024xf32) <- (-1x12544xf32, 12544x1024xf32)
        t35 = paddle._C_ops.matmul(t34, t6, False, False)
        del t6
        
        # pd_op.add: (-1x1024xf32) <- (-1x1024xf32, 1024xf32)
        t36 = paddle._C_ops.add(t35, t7)
        del t7
        
        # pd_op.relu: (-1x1024xf32) <- (-1x1024xf32)
        t37 = paddle._C_ops.relu(t36)
        del t36
        
        # pd_op.matmul: (-1x1024xf32) <- (-1x1024xf32, 1024x1024xf32)
        t38 = paddle._C_ops.matmul(t37, t8, False, False)
        del t8
        
        # pd_op.add: (-1x1024xf32) <- (-1x1024xf32, 1024xf32)
        t39 = paddle._C_ops.add(t38, t9)
        del t9
        
        # pd_op.relu: (-1x1024xf32) <- (-1x1024xf32)
        t40 = paddle._C_ops.relu(t39)
        del t39
        
        # pd_op.matmul: (-1x3xf32) <- (-1x1024xf32, 1024x3xf32)
        t41 = paddle._C_ops.matmul(t40, t10, False, False)
        del t10
        
        # pd_op.add: (-1x3xf32) <- (-1x3xf32, 3xf32)
        t42 = paddle._C_ops.add(t41, t11)
        del t11
        
        # pd_op.matmul: (-1x8xf32) <- (-1x1024xf32, 1024x8xf32)
        t43 = paddle._C_ops.matmul(t40, t12, False, False)
        del t12
        
        # pd_op.add: (-1x8xf32) <- (-1x8xf32, 8xf32)
        t44 = paddle._C_ops.add(t43, t13)
        del t13
        
        return t16, t17, t18, t19, t20, t21, t22, t23, t24, t25, t26, t27, t28, t29, t30, t32, t33, t34, t35, t37, t38, t40, t41, t42, t43, t44
    ...
    def get_input_spec(self):
        return [
            ...
            paddle.static.InputSpec(shape=[1024], dtype='float32'),
            # t10
            paddle.static.InputSpec(shape=[1024, 3], dtype='float32'),
            # t11
            paddle.static.InputSpec(shape=[3], dtype='float32'),
            # t12
            paddle.static.InputSpec(shape=[1024, 8], dtype='float32'),
            # t13
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            ...
            paddle.uniform([1024], dtype='float32', min=0, max=0.5),
            # t10
            paddle.uniform([1024, 3], dtype='float32', min=0, max=0.5),
            # t11
            paddle.to_tensor([0.0168662928044796, 0.40906161069869995, 0.05388816073536873], dtype='float32').reshape([3]),
            # t12
            paddle.uniform([1024, 8], dtype='float32', min=0, max=0.5),
            # t13
            paddle.to_tensor([0.23695418238639832, 0.1044808179140091, 0.1262960284948349, 0.392635703086853, 0.22426879405975342, 0.11570596694946289, 0.33996859192848206, 0.44903406500816345], dtype='float32').reshape([8]),
        ]
