...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, t18, t19):
        t1 = paddle._C_ops.full_int_array([0, -1, 384], paddle.int64, paddle.core.CPUPlace())
        t15 = paddle._C_ops.full_int_array([0, -1, 3, 12, 32], paddle.int64, paddle.core.CPUPlace())
        t16 = paddle._C_ops.full_int_array([0], paddle.int64, paddle.core.CPUPlace())
        t17 = paddle._C_ops.full_int_array([1], paddle.int64, paddle.core.CPUPlace())
        t18 = paddle._C_ops.full_int_array([2], paddle.int64, paddle.core.CPUPlace())
        t19 = paddle._C_ops.full_int_array([3], paddle.int64, paddle.core.CPUPlace())
        ...
        t22 = paddle._C_ops.matmul(t21, t2, False, False)
        del t2, t21
        
        # pd_op.add: (-1x-1x384xf32) <- (-1x-1x384xf32, 384xf32)
        t23 = paddle._C_ops.add(t22, t3)
        del t22, t3
        
        # pd_op.add: (-1x-1x384xf32) <- (-1x-1x384xf32, -1x-1x384xf32)
        t24 = paddle._C_ops.add(t4, t23)
        del t23, t4
        
        # pd_op.layer_norm: (-1x-1x384xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x384xf32, 384xf32, 384xf32)
        t25, t26, t27 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t24, t5, t6, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t24, t6, t5
        
        # pd_op.matmul: (-1x-1x1536xf32) <- (-1x-1x384xf32, 384x1536xf32)
        t28 = paddle._C_ops.matmul(t25, t7, False, False)
        del t7
        
        # pd_op.add: (-1x-1x1536xf32) <- (-1x-1x1536xf32, 1536xf32)
        t29 = paddle._C_ops.add(t28, t8)
        del t28, t8
        
        # pd_op.gelu: (-1x-1x1536xf32) <- (-1x-1x1536xf32)
        t30 = paddle._C_ops.gelu(t29, False)
        del t29
        
        # pd_op.matmul: (-1x-1x384xf32) <- (-1x-1x1536xf32, 1536x384xf32)
        t31 = paddle._C_ops.matmul(t30, t9, False, False)
        del t30, t9
        
        # pd_op.add: (-1x-1x384xf32) <- (-1x-1x384xf32, 384xf32)
        t32 = paddle._C_ops.add(t31, t10)
        del t31, t10
        
        # pd_op.add: (-1x-1x384xf32) <- (-1x-1x384xf32, -1x-1x384xf32)
        t33 = paddle._C_ops.add(t25, t32)
        del t32, t25
        
        # pd_op.layer_norm: (-1x-1x384xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x384xf32, 384xf32, 384xf32)
        t34, t35, t36 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t33, t11, t12, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t33, t12, t11
        
        # pd_op.matmul: (-1x-1x1152xf32) <- (-1x-1x384xf32, 384x1152xf32)
        t37 = paddle._C_ops.matmul(t34, t13, False, False)
        del t13
        
        # pd_op.add: (-1x-1x1152xf32) <- (-1x-1x1152xf32, 1152xf32)
        t38 = paddle._C_ops.add(t37, t14)
        del t37, t14
        
        # pd_op.reshape: (-1x-1x3x12x32xf32) <- (-1x-1x1152xf32, 5xi64)
        t39 = paddle._C_ops.reshape(t38, t15)
        del t38, t15
        
        # pd_op.transpose: (3x-1x12x-1x32xf32) <- (-1x-1x3x12x32xf32)
        t40 = paddle._C_ops.transpose(t39, [2, 0, 3, 1, 4])
        del t39
        
        # pd_op.slice: (-1x12x-1x32xf32) <- (3x-1x12x-1x32xf32, 1xi64, 1xi64)
        t41 = paddle._C_ops.slice(t40, [0], t16, t17, [1], [0])
        
        # pd_op.slice: (-1x12x-1x32xf32) <- (3x-1x12x-1x32xf32, 1xi64, 1xi64)
        t42 = paddle._C_ops.slice(t40, [0], t17, t18, [1], [0])
        
        # pd_op.slice: (-1x12x-1x32xf32) <- (3x-1x12x-1x32xf32, 1xi64, 1xi64)
        t43 = paddle._C_ops.slice(t40, [0], t18, t19, [1], [0])
        del t40
        
        # pd_op.transpose: (-1x12x32x-1xf32) <- (-1x12x-1x32xf32)
        t44 = paddle._C_ops.transpose(t42, [0, 1, 3, 2])
        del t42
        
        # pd_op.matmul: (-1x12x-1x-1xf32) <- (-1x12x-1x32xf32, -1x12x32x-1xf32)
        t45 = paddle._C_ops.matmul(t41, t44, False, False)
        del t41, t44
        
        # pd_op.scale: (-1x12x-1x-1xf32) <- (-1x12x-1x-1xf32, 1xf32)
        t46 = paddle._C_ops.scale(t45, t20, float('0'), True)
        del t45
        
        # pd_op.softmax: (-1x12x-1x-1xf32) <- (-1x12x-1x-1xf32)
        t47 = paddle._C_ops.softmax(t46, -1)
        del t46
        
        # pd_op.matmul: (-1x12x-1x32xf32) <- (-1x12x-1x-1xf32, -1x12x-1x32xf32)
        t48 = paddle._C_ops.matmul(t47, t43, False, False)
        del t43, t47
        
        # pd_op.transpose: (-1x-1x12x32xf32) <- (-1x12x-1x32xf32)
        t49 = paddle._C_ops.transpose(t48, [0, 2, 1, 3])
        del t48
    ...
    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[None, None, 12, 32], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([8, 240, 12, 32], dtype='float32', min=0, max=0.5),
            # t1
            paddle.to_tensor([0, -1, 384], dtype='int64').reshape([3]),
            # t2
            paddle.uniform([384, 384], dtype='float32', min=0, max=0.5),
            # t3
            paddle.uniform([384], dtype='float32', min=0, max=0.5),
            # t4
            paddle.uniform([8, 240, 384], dtype='float32', min=0, max=0.5),
            # t5
            paddle.uniform([384], dtype='float32', min=0, max=0.5),
            # t6
            paddle.uniform([384], dtype='float32', min=0, max=0.5),
            ...
            # t15
            paddle.to_tensor([0, -1, 3, 12, 32], dtype='int64').reshape([5]),
            # t16
