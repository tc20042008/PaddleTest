...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7):
        t1 = paddle._C_ops.full_int_array([2, 4725, 8, 32], paddle.int64, paddle.core.CPUPlace())
        # pd_op.reshape: (2x4725x8x32xf32) <- (2x4725x256xf32, 4xi64)
        t8 = paddle._C_ops.reshape(t0, t1)
        del t1
        
        # pd_op.matmul: (2x498x192xf32) <- (2x498x256xf32, 256x192xf32)
        t9 = paddle._C_ops.matmul(t2, t3, False, False)
        del t3
        
        # pd_op.add: (2x498x192xf32) <- (2x498x192xf32, 192xf32)
        ...
        # pd_op.reshape: (2x498x8x3x4x2xf32) <- (2x498x192xf32, 6xi64)
        t12 = paddle._C_ops.reshape(t10, t11)
        del t11
        
        # pd_op.matmul: (2x498x96xf32) <- (2x498x256xf32, 256x96xf32)
        t13 = paddle._C_ops.matmul(t2, t5, False, False)
        del t5
        
        # pd_op.add: (2x498x96xf32) <- (2x498x96xf32, 96xf32)
        ...
        # pd_op.reshape: (2x498x8x12xf32) <- (2x498x96xf32, 4xi64)
        t16 = paddle._C_ops.reshape(t14, t15)
        del t15
        
        # pd_op.softmax: (2x498x8x12xf32) <- (2x498x8x12xf32)
        ...
        # pd_op.reshape: (2x498x8x3x4xf32) <- (2x498x8x12xf32, 5xi64)
        ...
        # pd_op.slice: (2x498x1x2xf32) <- (2x498x1x4xf32, 1xi64, 1xi64)
        t25 = paddle._C_ops.slice(t7, [3], t20, t22, [1], [])
        
        # pd_op.full_int_array: (2xi64) <- ()
        t26 = [2, 4]
        
        # pd_op.unsqueeze: (2x498x1x1x1x2xf32) <- (2x498x1x2xf32, 2xi64)
        ...
        # pd_op.scale: (2x498x8x3x4x2xf32) <- (2x498x8x3x4x2xf32, 1xf32)
        ...
        # pd_op.multiply: (2x498x8x3x4x2xf32) <- (2x498x8x3x4x2xf32, 2x498x1x1x1x2xf32)
        ...
        # pd_op.add: (2x498x8x3x4x2xf32) <- (2x498x1x1x1x2xf32, 2x498x8x3x4x2xf32)
    ...
    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[2, 4725, 256], dtype='float32'),
            # t1
            ...
            # t7
            paddle.static.InputSpec(shape=[2, 498, 1, 4], dtype='float32'),
        ]
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([2, 4725, 256], dtype='float32', min=0, max=0.5),
            # t1
            paddle.to_tensor([2, 4725, 8, 32], dtype='int64').reshape([4]),
            # t2
            ...
            # t7
            paddle.uniform([2, 498, 1, 4], dtype='float32', min=0, max=0.5),
        ]
