...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11):
        ...
        t12 = paddle._C_ops.reshape(t0, t1)
        del t0
        
        # pd_op.add: (-1x128x-1x80xf32) <- (-1x128x-1x80xf32, 1x128x1x1xf32)
        t13 = paddle._C_ops.add(t2, t12)
        del t2, t12
        ...
        t15 = paddle._C_ops.flatten(t14, 2, 3)
        del t14
        
        # pd_op.transpose: (-1x-1x128xf32) <- (-1x128x-1xf32)
        t16 = paddle._C_ops.transpose(t15, [0, 2, 1])
        del t15
        
        # pd_op.layer_norm: (-1x-1x128xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x128xf32, 128xf32, 128xf32)
        t17, t18, t19 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t16, t4, t5, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t5, t4, t16
        ...
        t23 = paddle._C_ops.matmul(t22, t8, False, False)
        del t22, t8
        
        # pd_op.add: (-1x-1x128xf32) <- (-1x-1x128xf32, 128xf32)
        t24 = paddle._C_ops.add(t23, t9)
        del t23, t9
        
        # pd_op.add: (-1x-1x128xf32) <- (-1x-1x128xf32, -1x-1x128xf32)
        t25 = paddle._C_ops.add(t17, t24)
        del t24, t17
        
        # pd_op.layer_norm: (-1x-1x128xf32, -1x-1xf32, -1x-1xf32) <- (-1x-1x128xf32, 128xf32, 128xf32)
        t26, t27, t28 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t25, t10, t11, float('1e-06'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t25, t11, t10
        
        # pd_op.transpose: (-1x128x-1xf32) <- (-1x-1x128xf32)
        t29 = paddle._C_ops.transpose(t26, [0, 2, 1])
        del t26
    ...
    def get_input_spec(self):
        return [
            ...
            # t3
            paddle.static.InputSpec(shape=[None, None, None, 80], dtype='float32'),
            # t4
            paddle.static.InputSpec(shape=[128], dtype='float32'),
            # t5
            paddle.static.InputSpec(shape=[128], dtype='float32'),
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            ...
            # t2
            paddle.uniform([8, 128, 12, 80], dtype='float32', min=0, max=0.5),
            # t3
            paddle.uniform([8, 128, 12, 80], dtype='float32', min=0, max=0.5),
            # t4
            paddle.uniform([128], dtype='float32', min=0, max=0.5),
