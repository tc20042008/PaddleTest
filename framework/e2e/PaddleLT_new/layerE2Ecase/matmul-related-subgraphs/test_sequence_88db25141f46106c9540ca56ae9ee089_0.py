...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1):
        ...
        t2 = paddle._C_ops.transpose(t0, [0, 1, 3, 2])
        del t0
        
        # pd_op.matmul: (32x12x197x197xf32) <- (32x12x197x64xf32, 32x12x64x197xf32)
        t3 = paddle._C_ops.matmul(t1, t2, False, False)
    ...
    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[32, 12, 197, 64], dtype='float32'),
            # t1
            paddle.static.InputSpec(shape=[32, 12, 197, 64], dtype='float32'),
        ]
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([32, 12, 197, 64], dtype='float32', min=0, max=0.5),
            # t1
            paddle.uniform([32, 12, 197, 64], dtype='float32', min=0, max=0.5),
        ]
