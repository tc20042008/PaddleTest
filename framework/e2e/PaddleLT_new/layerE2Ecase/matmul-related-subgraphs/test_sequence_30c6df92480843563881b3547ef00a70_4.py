...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, ):
        ...
        # pd_op.full: (1xf64) <- ()
        t1 = paddle._C_ops.full([1], float('15'), paddle.float64, paddle.core.CPUPlace())
        ...
        # pd_op.flatten: (225xf32) <- (15x15xf32)
        ...
        # pd_op.unsqueeze: (225x1xf32) <- (225xf32, 1xi64)
        ...
        # pd_op.matmul: (225x96xf32) <- (225x1xf32, 1x96xf32)
        ...
        # pd_op.sin: (225x96xf32) <- (225x96xf32)
        t25 = paddle._C_ops.sin(t21)
        
        # pd_op.cos: (225x96xf32) <- (225x96xf32)
        ...
        # builtin.combine: ([225x96xf32, 225x96xf32, 225x96xf32, 225x96xf32]) <- (225x96xf32, 225x96xf32, 225x96xf32, 225x96xf32)
        t30 = [t25, t26, t27, t28]
        del t26, t28, t25, t27
        
        # pd_op.concat: (225x384xf32) <- ([225x96xf32, 225x96xf32, 225x96xf32, 225x96xf32], 1xi32)
        t31 = paddle._C_ops.concat(t30, t29)
        del t30, t29
        
        # pd_op.unsqueeze: (1x225x384xf32) <- (225x384xf32, 1xi64)
