...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3):
        t1 = paddle._C_ops.full_int_array([-1, 49, 96], paddle.int64, paddle.core.CPUPlace())
    ...
    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[4096, 7, 7, 96], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([4096, 7, 7, 96], dtype='float32', min=0, max=0.5),
            # t1
            paddle.to_tensor([-1, 49, 96], dtype='int64').reshape([3]),
            # t2
