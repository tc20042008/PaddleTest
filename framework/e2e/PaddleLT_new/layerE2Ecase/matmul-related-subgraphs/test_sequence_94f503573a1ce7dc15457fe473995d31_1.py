...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15):
        ...
        # pd_op.add: (-1x384x48x48xf32) <- (-1x384x48x48xf32, 1x384x1x1xf32)
        t18 = paddle._C_ops.add(t2, t17)
        del t2, t17
        
        # pd_op.transpose: (-1x48x48x384xf32) <- (-1x384x48x48xf32)
        t19 = paddle._C_ops.transpose(t18, [0, 2, 3, 1])
        del t18
        
        # pd_op.layer_norm: (-1x48x48x384xf32, -1x48x48xf32, -1x48x48xf32) <- (-1x48x48x384xf32, 384xf32, 384xf32)
        ...
        # pd_op.matmul: (-1x48x48x1536xf32) <- (-1x48x48x384xf32, 384x1536xf32)
        t23 = paddle._C_ops.matmul(t20, t5, False, False)
        del t20, t5
        
        # pd_op.add: (-1x48x48x1536xf32) <- (-1x48x48x1536xf32, 1536xf32)
        t24 = paddle._C_ops.add(t23, t6)
        del t23, t6
        
        # pd_op.gelu: (-1x48x48x1536xf32) <- (-1x48x48x1536xf32)
        t25 = paddle._C_ops.gelu(t24, False)
        del t24
        
        # pd_op.matmul: (-1x48x48x384xf32) <- (-1x48x48x1536xf32, 1536x384xf32)
        t26 = paddle._C_ops.matmul(t25, t7, False, False)
        del t25, t7
        
        # pd_op.add: (-1x48x48x384xf32) <- (-1x48x48x384xf32, 384xf32)
        t27 = paddle._C_ops.add(t26, t8)
        del t26, t8
        
        # pd_op.multiply: (-1x48x48x384xf32) <- (384xf32, -1x48x48x384xf32)
        t28 = paddle._C_ops.multiply(t9, t27)
        del t27, t9
        
        # pd_op.transpose: (-1x384x48x48xf32) <- (-1x48x48x384xf32)
        t29 = paddle._C_ops.transpose(t28, [0, 3, 1, 2])
        del t28
        
        # pd_op.add: (-1x384x48x48xf32) <- (-1x384x48x48xf32, -1x384x48x48xf32)
        t30 = paddle._C_ops.add(t10, t29)
        del t10, t29
        
        # pd_op.mean: (-1x1x48x48xf32) <- (-1x384x48x48xf32)
        t31 = paddle._C_ops.mean(t30, [1], True)
        
        # pd_op.subtract: (-1x384x48x48xf32) <- (-1x384x48x48xf32, -1x1x48x48xf32)
        t32 = paddle._C_ops.subtract(t30, t31)
        del t30, t31
        
        # pd_op.pow: (-1x384x48x48xf32) <- (-1x384x48x48xf32)
        ...
        # pd_op.scale: (-1x1x48x48xf32) <- (-1x1x48x48xf32, 1xf32)
        t35 = paddle._C_ops.scale(t34, t16, float('1e-06'), True)
        del t34
        
        # pd_op.sqrt: (-1x1x48x48xf32) <- (-1x1x48x48xf32)
        t36 = paddle._C_ops.sqrt(t35)
        del t35
        
        # pd_op.divide: (-1x384x48x48xf32) <- (-1x384x48x48xf32, -1x1x48x48xf32)
        ...
        # pd_op.multiply: (-1x384x48x48xf32) <- (384x1x1xf32, -1x384x48x48xf32)
        ...
        # pd_op.add: (-1x384x48x48xf32) <- (-1x384x48x48xf32, 384x1x1xf32)
        t41 = paddle._C_ops.add(t39, t40)
        del t39, t40
        
        # pd_op.conv2d: (-1x768x24x24xf32) <- (-1x384x48x48xf32, 768x384x2x2xf32)
    ...
    def get_input_spec(self):
        return [
            ...
            # t2
            paddle.static.InputSpec(shape=[None, 384, 48, 48], dtype='float32'),
            # t3
            paddle.static.InputSpec(shape=[384], dtype='float32'),
            # t4
            paddle.static.InputSpec(shape=[384], dtype='float32'),
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            ...
            # t2
            paddle.uniform([128, 384, 48, 48], dtype='float32', min=0, max=0.5),
            # t3
            paddle.uniform([384], dtype='float32', min=0, max=0.5),
            # t4
            paddle.uniform([384], dtype='float32', min=0, max=0.5),
