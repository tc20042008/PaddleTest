...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11):
        ...
        t15 = paddle._C_ops.shape64(t1)
        
        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        t16 = paddle._C_ops.slice(t15, [0], t2, t3, [1], [0])
        del t15
        ...
        t19 = paddle._C_ops.uniform(t18, paddle.float32, t12, t13, 0, paddle.framework._current_expected_place())
        del t18
        
        # pd_op.add: (-1x1x1xf32) <- (xf32, -1x1x1xf32)
        t20 = paddle._C_ops.add(t0, t19)
        del t19
        
        # pd_op.floor: (-1x1x1xf32) <- (-1x1x1xf32)
        t21 = paddle._C_ops.floor(t20)
        del t20
        
        # pd_op.divide: (-1x196x384xf32) <- (-1x196x384xf32, xf32)
        t22 = paddle._C_ops.divide(t1, t0)
        
        # pd_op.multiply: (-1x196x384xf32) <- (-1x196x384xf32, -1x1x1xf32)
        t23 = paddle._C_ops.multiply(t22, t21)
        
        # pd_op.add: (-1x196x384xf32) <- (-1x196x384xf32, -1x196x384xf32)
        t24 = paddle._C_ops.add(t5, t23)
        
        # pd_op.layer_norm: (-1x196x384xf32, -1x196xf32, -1x196xf32) <- (-1x196x384xf32, 384xf32, 384xf32)
        t25, t26, t27 = (lambda x, f: f(x))(paddle._C_ops.layer_norm(t24, t6, t7, float('1e-05'), 2), lambda out: out if isinstance(out, (list, tuple)) else (out, None,None))
        del t7, t6
        
        # pd_op.matmul: (-1x196x1536xf32) <- (-1x196x384xf32, 384x1536xf32)
        t28 = paddle._C_ops.matmul(t25, t8, False, False)
        del t8
        
        # pd_op.add: (-1x196x1536xf32) <- (-1x196x1536xf32, 1536xf32)
        t29 = paddle._C_ops.add(t28, t9)
        del t9
        ...
        t31 = paddle._C_ops.matmul(t30, t10, False, False)
        del t10
        
        # pd_op.add: (-1x196x384xf32) <- (-1x196x384xf32, 384xf32)
        t32 = paddle._C_ops.add(t31, t11)
        del t11
        ...
        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        t34 = paddle._C_ops.slice(t33, [0], t2, t3, [1], [0])
        del t33
        ...
        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        t36 = paddle._C_ops.stack(t35, 0)
        del t35
        
        # pd_op.uniform: (-1x1x1xf32) <- (3xi64, 1xf32, 1xf32)
        t37 = paddle._C_ops.uniform(t36, paddle.float32, t12, t13, 0, paddle.framework._current_expected_place())
        del t36
        
        # pd_op.add: (-1x1x1xf32) <- (xf32, -1x1x1xf32)
        t38 = paddle._C_ops.add(t0, t37)
        del t37
        
        # pd_op.floor: (-1x1x1xf32) <- (-1x1x1xf32)
        t39 = paddle._C_ops.floor(t38)
        del t38
        
        # pd_op.divide: (-1x196x384xf32) <- (-1x196x384xf32, xf32)
        t40 = paddle._C_ops.divide(t32, t0)
        
        # pd_op.multiply: (-1x196x384xf32) <- (-1x196x384xf32, -1x1x1xf32)
        t41 = paddle._C_ops.multiply(t40, t39)
        
        # pd_op.add: (-1x196x384xf32) <- (-1x196x384xf32, -1x196x384xf32)
        t42 = paddle._C_ops.add(t24, t41)
        
        return t14, t21, t22, t23, t24, t25, t26, t27, t28, t29, t30, t31, t32, t39, t40, t41, t42
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.to_tensor(0.8727269768714905, dtype='float32').reshape([]),
            # t1
