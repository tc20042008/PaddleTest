...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9):
        t1 = paddle._C_ops.full_int_array([1024, 49, 3, 6, 32], paddle.int64, paddle.core.CPUPlace())
        ...
        t5 = paddle._C_ops.full_int_array([3], paddle.int64, paddle.core.CPUPlace())
        t7 = paddle._C_ops.full_int_array([-1], paddle.int64, paddle.core.CPUPlace())
        t9 = paddle._C_ops.full_int_array([49, 49, -1], paddle.int64, paddle.core.CPUPlace())
        ...
        t13 = paddle._C_ops.slice(t12, [0], t2, t3, [1], [0])
        
        # pd_op.slice: (1024x6x49x32xf32) <- (3x1024x6x49x32xf32, 1xi64, 1xi64)
        t14 = paddle._C_ops.slice(t12, [0], t3, t4, [1], [0])
        
        # pd_op.slice: (1024x6x49x32xf32) <- (3x1024x6x49x32xf32, 1xi64, 1xi64)
        t15 = paddle._C_ops.slice(t12, [0], t4, t5, [1], [0])
        
        # pd_op.scale: (1024x6x49x32xf32) <- (1024x6x49x32xf32, 1xf32)
        t16 = paddle._C_ops.scale(t13, t10, float('0'), True)
        del t13
        
        # pd_op.transpose: (1024x6x32x49xf32) <- (1024x6x49x32xf32)
        t17 = paddle._C_ops.transpose(t14, [0, 1, 3, 2])
        del t14
        
        # pd_op.matmul: (1024x6x49x49xf32) <- (1024x6x49x32xf32, 1024x6x32x49xf32)
        t18 = paddle._C_ops.matmul(t16, t17, False, False)
        
        # pd_op.reshape: (2401xi64) <- (49x49xi64, 1xi64)
        t19 = paddle._C_ops.reshape(t6, t7)
        del t6
        
        # pd_op.index_select: (2401x6xf32) <- (169x6xf32, 2401xi64)
        t20 = paddle._C_ops.index_select(t8, t19, 0)
        del t8
        
        # pd_op.reshape: (49x49x6xf32) <- (2401x6xf32, 3xi64)
        t21 = paddle._C_ops.reshape(t20, t9)
        
        # pd_op.transpose: (6x49x49xf32) <- (49x49x6xf32)
        t22 = paddle._C_ops.transpose(t21, [2, 0, 1])
        del t21
        
        # pd_op.unsqueeze: (1x6x49x49xf32) <- (6x49x49xf32, 1xi64)
        t23 = paddle._C_ops.unsqueeze(t22, t2)
        
        # pd_op.add: (1024x6x49x49xf32) <- (1024x6x49x49xf32, 1x6x49x49xf32)
        t24 = paddle._C_ops.add(t18, t23)
        ...
        t26 = paddle._C_ops.matmul(t25, t15, False, False)
        
        # pd_op.transpose: (1024x49x6x32xf32) <- (1024x6x49x32xf32)
        t27 = paddle._C_ops.transpose(t26, [0, 2, 1, 3])
        del t26
        
        return t12, t15, t16, t17, t18, t19, t20, t22, t23, t25, t27

    def get_input_spec(self):
        return [
            # t0
            paddle.static.InputSpec(shape=[1024, 49, 576], dtype='float32'),
            # t1
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([1024, 49, 576], dtype='float32', min=0, max=0.5),
            # t1
