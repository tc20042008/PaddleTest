...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4, t5):
        t1 = paddle._C_ops.full_int_array([0, -1, 3, 12, 32], paddle.int64, paddle.core.CPUPlace())
        ...
        t6 = paddle._C_ops.reshape(t0, t1)
        del t0, t1
        
        # pd_op.transpose: (3x-1x12x-1x32xf32) <- (-1x-1x3x12x32xf32)
        t7 = paddle._C_ops.transpose(t6, [2, 0, 3, 1, 4])
        del t6
        
        # pd_op.slice: (-1x12x-1x32xf32) <- (3x-1x12x-1x32xf32, 1xi64, 1xi64)
        t8 = paddle._C_ops.slice(t7, [0], t2, t3, [1], [0])
        
        # pd_op.slice: (-1x12x-1x32xf32) <- (3x-1x12x-1x32xf32, 1xi64, 1xi64)
        t9 = paddle._C_ops.slice(t7, [0], t3, t4, [1], [0])
        
        # pd_op.slice: (-1x12x-1x32xf32) <- (3x-1x12x-1x32xf32, 1xi64, 1xi64)
        t10 = paddle._C_ops.slice(t7, [0], t4, t5, [1], [0])
        del t7
        
        # pd_op.transpose: (-1x12x32x-1xf32) <- (-1x12x-1x32xf32)
        t11 = paddle._C_ops.transpose(t9, [0, 1, 3, 2])
        del t9
        
        # pd_op.matmul: (-1x12x-1x-1xf32) <- (-1x12x-1x32xf32, -1x12x32x-1xf32)
        t12 = paddle._C_ops.matmul(t8, t11, False, False)
        del t8, t11
