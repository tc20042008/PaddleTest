...
class SubGraphLayer(InstanceTrait, paddle.nn.Layer):
    ...
    def forward(self, t0, t1, t2, t3, t4):
        # pd_op.matmul: (2x-1x384xf32) <- (2x-1x-1xf32, 96x384xf32)
        ...
        # pd_op.gelu: (2x-1x384xf32) <- (2x-1x384xf32)
        t7 = paddle._C_ops.gelu(t6, False)
        
        # pd_op.matmul: (2x-1x96xf32) <- (2x-1x384xf32, 384x96xf32)
        t8 = paddle._C_ops.matmul(t7, t3, False, False)
        del t3
        
        # pd_op.add: (2x-1x96xf32) <- (2x-1x96xf32, 96xf32)
...
class TestSubGraphLayer(CinnTestBase, unittest.TestCase):
    ...
    def get_inputs(self):
        return [
            # t0
            paddle.uniform([2, 37632, 96], dtype='float32', min=0, max=0.5),
            # t1
